...
export default function UniversityDetail({ university }: UniversityDetailProps) {
  const { image, name, country, qsRank, rating, description, about, programs } = university;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <section className="border border-zinc-100 rounded-2xl p-6">
            ...
            <p className="text-sm text-zinc-500 leading-relaxed">{about}</p>
          </section>
          ...
          <section className="border border-zinc-100 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {programs.map(({ id, name: programName, type }) => (
                <div
                  key={id}
                  className="flex items-center justify-between p-4 rounded-xl border border-zinc-100 hover:border-zinc-200 hover:bg-zinc-50/50 transition-all"
                >
                  <div>
                    <p className="text-sm font-medium text-zinc-800">{programName}</p>
                    <span className={`inline-block mt-1 text-xs py-0.5 font-medium text-[#7F838D]`}>
                      {TYPE_LABELS[type]}
                    </span>
                  </div>
                  <button className="text-xs border border-zinc-200 rounded-lg px-3 py-1.5 text-zinc-600 hover:bg-zinc-900 hover:text-white hover:border-zinc-900 transition-all duration-200 shrink-0 ml-4">
                    Подробнее
                  </button>
                </div>
              ))}
          ...
          <div className="border border-zinc-100 rounded-2xl p-6 sticky top-6">
            ...
            <div className="space-y-4 mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-zinc-400">Программ</p>
                  <p className="text-sm font-semibold text-zinc-800">{programs.length}+</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-zinc-800">{rating}/5</p>
                </div>
              </div>
            </div>
